...
export function Droppable(props) {
  ...
  const { isOver: subIsOver, setDroppableNodeRef: setSubDroppableNodeRef } =
    useSortable({
          ...
          {list.map(
            (shift) =>
              ...
              (filterQueryWorkGroup.length > 0
                ? filterQueryWorkGroup.some(
                    (val) => val.label === shift.workGroup
                  )
                : filterQueryWorkGroup) &&
              (filterQueryTimePeriod.length > 0
                ? filterQueryTimePeriod.some(
                    (val) => val.label === shift.timePeriod
                  )
                : filterQueryTimePeriod) &&
              (filterQueryOnlyShow.length > 0
                ? filterQueryOnlyShow.some(
                    (val) => val.label === shift.timePeriod
                  )
                : filterQueryOnlyShow) && (
                ...
                <div key={shift.id}>
                  ...
                  <div
                    ...
                    style={{
                      ...
                      ...((item.position === "Bartender" ||
                        shift.position === "Bartender") &&
                        !shift.isReleased &&
                        !shift.isHouse &&
                        !shift.isPriority && {
                      ...
                      ...((item.position === "Head Chef" ||
                        shift.position === "Head Chef") &&
                        !shift.isReleased &&
                        !shift.isHouse &&
                        !shift.isPriority && {
